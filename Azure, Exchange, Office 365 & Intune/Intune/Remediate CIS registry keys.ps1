<#
.SYNOPSIS
Script for use with Intune remedation tool to fix if registry keys have the correct value. 
...
.DESCRIPTION 
Script for use with Intune remedation tool to fix if registry keys have the correct value (detect by the detection script).  
...
This is the remedation script. If a regkey is not like stated below, this script will fix it.

.REQUIREMENTS
- At least Powershell V5

.INSTRUCTIONS
- Deploy this remedation script in Intune, setup the Intune part, and profit!
#>
...
Start-transcript "C:\Scripts\Logs\CIS registry keys-remediation.log" -Force
