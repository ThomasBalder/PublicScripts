...
-- Creates the mailbox;
-- Sets the correct mailboxpermissions (lines 140/141)
...
.INSTRUCTIONS
- Change the variable from line 40 for your organization;
- There are a few (generalized) functions & variables in this script that you might not use (i.e. line 90 & 143);
...
.NOTES 
- Note that sometimes it takes longer than two minutes to create the mailbox, so if the Accessrights assignment fails the first time
...
$mailboxuser1 = "joe@contoso.com"
$mailboxuser2 = "mary@contoso.com"

#Start logging
Start-Transcript -Path c:\temp\User-Creation.log -Force
...
New-aduser -samaccountname $adname -UserPrincipalName "$adname@$FQDN" -name $Name -Givenname $Firstname -Surname $Lastname -DisplayName $Name `
...
Write-Host "The mailbox should now be created. Let's assign the correct accessrights." -ForegroundColor Green
Add-MailboxPermission -Identity $ADName -User $mailboxuser1 -AccessRights FullAccess -Confirm:$false -AutoMapping $false
Add-MailboxPermission -Identity $ADName -User $mailboxuser2 -AccessRights FullAccess -Confirm:$false -AutoMapping $false
